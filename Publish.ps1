...
# Progress messages go to the information stream, so a caller can redirect or capture them.
# 'Continue' keeps them visible by default, which is what an interactive publish needs.
...
if ($runExitCode -ne 0) {
	$report = @("The release run did not succeed: https://github.com/$repoFullName/actions/runs/$runId")
	...
	if ($LASTEXITCODE -eq 0 -and $jobId) {
		...
		if ($LASTEXITCODE -eq 0 -and $annotation) {
			$report += "Reason: $annotation"
		}
	}

	$report += "Tag $version is pushed but no package was published. Once the cause is fixed:"
	$report += "  gh run rerun $runId --repo $repoFullName --failed"
