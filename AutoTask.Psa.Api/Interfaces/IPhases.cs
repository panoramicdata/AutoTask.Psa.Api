...
public interface IPhases
{
	...
	Task<QueryActionResult<PhaseModel>> QueryAsync([Body] QueryModel queryModel, CancellationToken cancellationToken);
	...
	/// <returns>Task of PhaseModel</returns>
	[Get("/V1.0/Phases/{id}")]
	Task<PhaseModel> GetAsync([AliasAs("id")] long id, CancellationToken cancellationToken);
	...
	Task<QueryActionResult<PhaseModel>> QueryAsync([AliasAs("search")] string search, CancellationToken cancellationToken);
