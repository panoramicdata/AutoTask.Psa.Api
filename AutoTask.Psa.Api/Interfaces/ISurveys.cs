...
public interface ISurveys
{
	...
	Task<QueryActionResult<SurveyModel>> QueryAsync([Body] QueryModel queryModel, CancellationToken cancellationToken);
	...
	/// <returns>Task of SurveyModel</returns>
	[Get("/V1.0/Surveys/{id}")]
	Task<SurveyModel> GetAsync([AliasAs("id")] long id, CancellationToken cancellationToken);
	...
	Task<QueryActionResult<SurveyModel>> QueryAsync([AliasAs("search")] string search, CancellationToken cancellationToken);
